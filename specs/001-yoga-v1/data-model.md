...
users
 ...
 ├── instructor_students (many-to-many — a student can link to multiple instructors)
...
| status | text | 'sent', 'retrying', 'failed' |
...
| Column | Type | Notes |
|---|---|---|
| instructor_id | uuid | FK → users.id, PK (composite) |
| student_id | uuid | FK → users.id, PK (composite) |
| linked_via | text | 'invite' or 'discovery' |
...
| instructor_students | — | SELECT own | SELECT + INSERT for linked students |
...
| 003 | Create instructor_students, bookings, notifications_log, student_notification_prefs |
